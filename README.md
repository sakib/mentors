...
This is an API that exposes Rutgers tech mentors on basic endpoints in JSON format.
...
```
mysql> create database mentors;
mysql> create user 'mentors'@'localhost' identified by 'mentors';
mysql> grant all privileges on mentors.* to 'mentors'@'localhost';
```
...
For most table changes, edit mentors/models.py, and run the above migrate and upgrade commands again.
